...
export default function PhoneEntryPage() {
   ...
   * create-account step — otherwise t("login.google") is indistinguishable
      ...
      <div className="flex w-full max-w-[390px] flex-1 flex-col px-6 pb-8 pt-6">
        ...
        {googleAccount && (
          <div className="mt-6 flex items-center gap-2 rounded-[12px] border border-[#BBF7D0] bg-[#F0FDF4] px-3.5 py-3">
            ...
            <span className="min-w-0 flex-1 text-[13px] text-text-primary">{t("phone.signedInAs")}<span className="font-semibold">{googleAccount}</span>
            </span>
          </div>
        )}
        ...
        <div className="mt-11">
          ...
          {value.length > 0 && !isValid && (
            <p className="mt-2 text-sm text-error">{t("phone.invalid")}</p>
          )}
        </div>
        ...
        <div className="mt-auto pt-8">
          <Button disabled={!isValid} loading={loading} onClick={handleContinue}>{t("common.continue")}</Button>
        </div>
      </div>
    </div>
  );
